...
-- Maker preference table (FIXED: industry_id should be UUID, not SMALLINT)
...
-- Job license junction table (FIXED: license_id should be UUID, not SMALLINT)
...
-- Maker work experience policies
CREATE POLICY "Users can view their own work experience" ON maker_work_experience
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own work experience" ON maker_work_experience
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own work experience" ON maker_work_experience
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own work experience" ON maker_work_experience
    FOR DELETE USING (auth.uid() = user_id);

-- Maker preference policies
CREATE POLICY "Users can view their own preferences" ON maker_preference
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own preferences" ON maker_preference
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own preferences" ON maker_preference
    FOR UPDATE USING (auth.uid() = user_id);

-- Maker license policies
CREATE POLICY "Users can view their own licenses" ON maker_license
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own licenses" ON maker_license
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete their own licenses" ON maker_license
    FOR DELETE USING (auth.uid() = user_id);

-- Maker reference policies
CREATE POLICY "Users can view their own references" ON maker_reference
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own references" ON maker_reference
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own references" ON maker_reference
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own references" ON maker_reference
    FOR DELETE USING (auth.uid() = user_id);

-- Maker visa policies
CREATE POLICY "Users can view their own visas" ON maker_visa
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own visas" ON maker_visa
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own visas" ON maker_visa
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own visas" ON maker_visa
    FOR DELETE USING (auth.uid() = user_id);

-- Employer facility policies
CREATE POLICY "Users can view their own facilities" ON employer_facility
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own facilities" ON employer_facility
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete their own facilities" ON employer_facility
    FOR DELETE USING (auth.uid() = user_id);

-- Employer job type policies
CREATE POLICY "Users can view their own job types" ON employer_job_type
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own job types" ON employer_job_type
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete their own job types" ON employer_job_type
    FOR DELETE USING (auth.uid() = user_id);

-- Job license policies
CREATE POLICY "Public can view job licenses" ON job_license
    FOR SELECT USING (true);

CREATE POLICY "Employers can insert job licenses for their jobs" ON job_license
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM job 
            WHERE job.job_id = job_license.job_id 
            AND job.user_id = auth.uid()
        )
    );

CREATE POLICY "Employers can delete job licenses for their jobs" ON job_license
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM job 
            WHERE job.job_id = job_license.job_id 
...
CREATE POLICY "Public can view industries" ON industry FOR SELECT USING (true);
CREATE POLICY "Public can view industry roles" ON industry_role FOR SELECT USING (true);
CREATE POLICY "Public can view facilities" ON facility FOR SELECT USING (true);
CREATE POLICY "Public can view job types" ON job_type FOR SELECT USING (true);
CREATE POLICY "Public can view licenses" ON license FOR SELECT USING (true);
