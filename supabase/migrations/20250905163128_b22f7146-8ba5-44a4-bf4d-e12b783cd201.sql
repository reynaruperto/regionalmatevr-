-- Enable UUID extension (optional, gen_random_uuid() is built-in for PostgreSQL 13+)
...
CREATE TABLE whv_maker (
    ...
    profile_photo TEXT, -- Storage path: e.g., 'profile_photos/user_id/filename.jpg'
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE maker_preference (
    ...
    state state,
    suburb_city TEXT,
    industry_id SMALLINT REFERENCES industry(industry_id),
    industry_role_id SMALLINT REFERENCES industry_role(industry_role_id)
);

-- Maker license junction table
CREATE TABLE maker_license (
    user_id UUID NOT NULL REFERENCES profile(user_id) ON DELETE CASCADE,
    license_id SMALLINT NOT NULL REFERENCES license(license_id) ON DELETE CASCADE,
    other TEXT,
...
CREATE POLICY "Users can manage their own visas" ON maker_visa
    FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Job owners can manage job licenses" ON job_license
    FOR ALL USING (EXISTS (
        SELECT 1 FROM job WHERE job.job_id = job_license.job_id AND job.user_id = auth.uid()
    ));
...
CREATE POLICY "Users can view their own profile photos" ON storage.objects
    FOR SELECT USING (bucket_id = 'profile_photos' AND auth.uid()::text = (storage.foldername(name))[1]);
...
BEGIN
    ...
    RETURN 'https://xiymsnmlwffikkhwatcp.supabase.co/storage/v1/object/public/profile_photos/' || path;
END;
